...
def main() -> int:
    ...
    require("alignListItems" in text, "list item text must be aligned with the inherited list direction")
    require("SIDEBAR_LABEL_SELECTOR" in text, "sidebar leaf-label direction handling is missing")
    require("SIDEBAR_SECTION_LABEL_SELECTOR" in text, "sidebar section-heading exclusion is missing")
    require("SYSTEM_UI_STACK" in text, "sidebar section headings need a native UI font fallback")
    require("item.style.setProperty('unicode-bidi', 'isolate')" in text, "list items must override Hermes plaintext bidi")
